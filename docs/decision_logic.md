...
- Riceve `avg_entry_price` e `unrealized_pnl_pct` direttamente nel `PortfolioState`: il runner li calcola a ogni ciclo con metodo FIFO sui lotti BUY ancora aperti. Il DM li usa come riferimento concreto per le decisioni di uscita: se `unrealized_pnl_pct` è in territorio positivo valuta se prendere un take profit parziale invece di accumulare ulteriormente; se è in territorio negativo valuta se aggiungere nuove tranche è giustificato dal setup o se sta mediando al ribasso senza ragione. Entrambi i campi sono `None` se non c'è posizione aperta.
