...
- Riceve `avg_entry_price` e `unrealized_pnl_pct` direttamente nel `PortfolioState`: il runner li calcola a ogni ciclo con metodo FIFO sui lotti BUY ancora aperti. Il DM li usa come riferimento concreto per le decisioni di uscita: se `unrealized_pnl_pct` è già significativamente positivo (es. ≥ +10/15%) valuta seriamente un take profit parziale invece di accumulare ulteriormente. Entrambi i campi sono `None` se non c'è posizione aperta.
