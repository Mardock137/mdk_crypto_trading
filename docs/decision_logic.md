...
- Riceve `avg_entry_price`, `unrealized_pnl_pct` e `unrealized_pnl_usdc` direttamente nel `PortfolioState`: il runner li calcola a ogni ciclo con metodo FIFO sui lotti BUY ancora aperti. Il DM li usa come riferimento concreto per le decisioni di uscita: se `unrealized_pnl_pct` è in territorio positivo valuta se prendere un take profit parziale invece di accumulare ulteriormente; se è in territorio negativo valuta se aggiungere nuove tranche è giustificato dal setup o se sta mediando al ribasso senza ragione. `unrealized_pnl_usdc` esprime lo stesso P&L in valore assoluto USDC, calcolato sulla quantità **tracciata dal FIFO** (`open_qty`) e non sul saldo totale dell'exchange: le monete non tracciate dalla memoria del bot hanno costo di carico ignoto e non vengono incluse. Il DM **non deve calcolare il P&L autonomamente**: se entrambi i campi sono `None`, non c'è posizione tracciabile. Tutti e tre i campi sono `None` se non c'è posizione aperta.
...
- Riceve `oco_review_required`: flag booleano calcolato deterministicamente dal runner. Diventa `True` quando un ordine OCO è aperto da più di `oco_review_interval_hours` (configurabile in `config/trading.yaml`, default 24h). Quando è `True`, il prompt impone una valutazione esplicita dei livelli TP/SL nella `reason`: il DM può confermarne la validità o proporre `CANCEL_AND_REPLACE_ORDER`, ma un HOLD silenzioso non è ammesso.
...
- Riceve anche `latest_news_review`: l'ultimo digest del `News Reviewer` (aggiornato ogni 12h), contenente sentiment complessivo (`BULLISH`/`BEARISH`/`NEUTRAL`), sintesi degli eventi rilevanti e risk flag. Va pesato come **contesto macro**, non come ordine automatico: il segnale primario resta l'analisi tecnica del Market Analyst. I `risk_flags` meritano attenzione particolare in quanto segnalano possibili shock di volatilità. Il campo è vuoto se la chiave Alpha Vantage non è configurata o il primo report non è ancora stato generato.
